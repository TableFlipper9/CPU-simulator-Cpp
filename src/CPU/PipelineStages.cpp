...
void IDStage::evaluate(PipelineRegisters& pipe, const RegisterFile& regs, bool stall) {
    if (stall) {
        // Insert NOPinto ID/EX, IF/ID is held by IF stage.
    ...
    // Keep the decoded instruction for debugg
    out.rawInstr = di;
...
// We resolve branches/jumps in EX.
